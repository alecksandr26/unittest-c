...
extern int compile_obj(const UnitCompiler *compiler_contex, const char *source,
		       const char *obj);

/* link_objs: To link the entire objects file into a testrunner, returns the return status
   of the linkage
   of the objects. */
extern int link_objs(const UnitCompiler *compiler_contex, const char *file,
		     const char **objs, size_t amount_objs, const char *out);
