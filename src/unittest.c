...
static void unittest_print_tests_results(double duration, size_t crashed_testcases,
					 size_t failed_testcases, size_t failed_tests,
					 size_t warned_tests, size_t count_tests)
{
	LOG("\n");
	...
	for (size_t i = 0; i < failed_testcases; i++)
		unittest_print_info_testcase(unittest_infotestcases[i]);

	LOG(DIV_LINE_STR);
	LOG("Ran %zu test in %fs\n", count_tests, duration);

	if (failed_tests == 0 && crashed_testcases == 0 && warned_tests == 0 &&
	    failed_testcases == 0) {
		LOG("\nOk \n\n");
		unittest_ret = 0;
	}
...
/* unittest_run_tests: Takes the linked list of testcases and run isolated it each
 * individual it. */
void unittest_run_tests(void)
{
	...
	failed_testcases = crashed_testcases = count_tests = successed_tests =
		failed_tests = warned_tests = 0;
	...
	while (unittest_head_tc != NULL) {
		...
		if (unittest_head_tc->sigstatus == 0 &&
			...
			successed_tests +=
				unittest_head_tc->amount -
				unittest_head_tc->info.number_failed_asserts -
				unittest_head_tc->info.number_warning_expects;

			/* Catch its failed asserts info */
			if (unittest_head_tc->info.number_failed_asserts ||
			    unittest_head_tc->info.number_warning_expects)
				unittest_infotestcases[failed_testcases++] =
					&unittest_head_tc->info;
			...
			unittest_info_crashed_testcases[crashed_testcases] =
				&unittest_head_tc->crashed_info;
			unittest_catch_info_crashed(
				unittest_info_crashed_testcases[crashed_testcases],
				unittest_head_tc);
			crashed_testcases++;
		}
	...
	/* Printing section */
	unittest_print_tests_results(duration, crashed_testcases, failed_testcases,
				     failed_tests, warned_tests, count_tests);
}
