...
/* To compute the amount of elements inside of the array */
#define NELEMS(x) ((sizeof(x)) / (sizeof((x)[0])))

/* The structure of our buckets table */
static struct atom {
	struct atom *link;
	int	     len;
	char	    *str;
} *buckets[TABLE_LENGTH] = {NULL};
...
{
	char	      str[43];
	char	     *s = str + sizeof str; /* Prepare the pointer */
	unsigned long m;
...
const char *Atom_new(const char *str, int len)
{
	...
	assert(str && "Can't be null the string");
	assert(len >= 0 && "The length of the of the string can't be lesser than 0");

	/* Compute a hash */
	h = Atom_hash(str) % NELEMS(buckets);

	for (p = buckets[h]; p; p = p->link)
		if (len == p->len) {
			for (i = 0; i < len && p->str[i] == str[i];)
				i++;
			if (i == len) return p->str; /* Return the actual entry */
		}
	...
	p->len = len;
	p->str = (char *) (p + 1); /* Move to the str space */

	if (len > 0) memcpy(p->str, (char *) str, len); /* Copy the source */

	p->str[len] = '\0';	  /* Null terminated */
	p->link	    = buckets[h]; /* Link to the buckets */
	buckets[h]  = p;	  /* Enqueue The new entry */

	return (const char *) p->str;
}

/* Atom_length: Trys to get the length from the atom. */
int Atom_length(const char *str)
{
	...
	for (p = buckets[h]; p; p = p->link)
		if (len == (size_t) p->len) {
			for (i = 0; i < (int) len && p->str[i] == str[i];)
				i++;
			if (i == (int) len) return len;
		}
...
void Atom_free(const char *str)
{
	...
	int	      i;
	const size_t  len = strlen(str);
	struct atom  *p, *prev; /* Where to stored the atom */
	...
	for (p = buckets[h]; p; p = p->link) {
		if (len == (size_t) p->len) {
			for (i = 0; i < (int) len && p->str[i] == str[i];)
				i++;
			if (i == (int) len) {
				if (prev != NULL) /* Modify the prev link */
					prev->link = p->link;
				else buckets[h] = p->link;
				free(p); /* Free the atom */
				return;
			}
		}
		prev = p;
	...
	assert(0); /* Can't dealloc an atom that doesn't exist */
}
